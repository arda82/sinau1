    ...
    while True:
        ...
        try:
            if pilihan == "1":
                ...
                # Meminta ID nilai yang ingin diedit
                id_edit = input("Masukkan ID data yang ingin diedit: ")

                # Mendapatkan nilai lama
                cursor.execute(f"SELECT * FROM {nama_tabel} WHERE {kolom_nama[0]} = ?", (id_edit,))
                ...
                if nilai_lama is None:
                    ...
                    # Memastikan jumlah nilai sama dengan jumlah kolom
                    if len(values) != len(kolom_nama):
                        print(f"Jumlah nilai ({len(values)}) tidak sama dengan jumlah kolom ({len(kolom_nama)})")

                    else:
                # Menyiapkan query UPDATE
                        query = f"UPDATE {nama_tabel} SET {','.join(f'{kolom_nama[i]} = ?' for i in range(len(kolom_nama)))} WHERE {kolom_nama[0]} = ?"

                        # Menjalankan query dengan nilai
                        values.append(id_edit)
                        cursor.execute(query, values)
                        taut.commit()
